...
export const Sidebar = () => {
  ...
  const toggleWorkspace = (workspaceId: string) => {
    setExpandedWorkspaces(prev => prev.includes(workspaceId) ? prev.filter(id => id !== workspaceId) : [...prev, workspaceId]);
  };
          ...
          <div className="flex-1 p-4 space-y-6 overflow-y-auto my-[50px]">
            {workspaces.map(workspace => <Collapsible key={workspace.id} open={expandedWorkspaces.includes(workspace.id)} onOpenChange={() => toggleWorkspace(workspace.id)}>
                ...
                <CollapsibleContent>
                  ...
                  return <Link key={workflow.id} to={`/${workspace.id}/${workflow.id}`} className={`flex items-center gap-2 text-sm py-1 px-2 rounded transition-colors ${isActive ? "bg-gray-100 text-gray-900" : "text-gray-500 hover:text-gray-900 hover:bg-gray-50"}`}>
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`${isActive ? "text-gray-900" : "text-gray-500"}`}>
